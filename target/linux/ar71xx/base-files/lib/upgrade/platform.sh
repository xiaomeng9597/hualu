...
platform_check_image_gl_ar300m_lite() {
	local board=$2
	local magic=$3

	if gl_ar300m_lite_is_nand; then
		nand_do_platform_check $board $1
		return $?
	else
		[ "$magic" != "2705" ] && {
...
platform_check_image() {
	...
	gl-ar300m)
		...
		;;
	gl-ar300m-lite)
		platform_check_image_gl_ar300m_lite "$1" "$board" "$magic" && return 0
		return 1
		;;
	alfa-ap96 | \
	alfa-nx | \
...
platform_pre_upgrade_gl_ar300m_lite() {
	if gl_ar300m_lite_is_nand; then
		nand_do_upgrade "$1"
	fi
}
...
platform_pre_upgrade() {
	...
	gl-ar300m)
		...
		;;
	gl-ar300m-lite)
		platform_pre_upgrade_gl_ar300m_lite "$1"
		;;
	nbg6716 | \
	r6100 | \
	wndr3700v4 | \
